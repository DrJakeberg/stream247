...
export async function listOverlayVideoSourceRecords(): Promise<OverlayVideoSourceRecord[]> {
  ...
  try {
    const result = await client.query<{
      ...
      // Presence is computed in SQL so the encrypted key never travels with a listing.
      "SELECT id, name, url_present, ingest_kind, encrypted_publish_key <> '' AS publish_key_present, updated_at FROM overlay_video_sources ORDER BY name, id"
    );
