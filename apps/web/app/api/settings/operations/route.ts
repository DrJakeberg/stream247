...
type OperationsBody = Partial<Record<(typeof FLAG_KEYS)[number] | (typeof PERCENT_KEYS)[number], string>>;
...
export async function PUT(request: NextRequest) {
  ...
  const touchesPair = PERCENT_KEYS.some((key) => updates[key] !== undefined && candidate[key] !== "");
  if (touchesPair) {
