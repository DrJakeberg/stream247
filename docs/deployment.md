...
`.env.production.example` pins `v1.5.2` for stable deployment.
...
- `ghcr.io/drjakeberg/stream247-web:v1.5.2`
- `ghcr.io/drjakeberg/stream247-worker:v1.5.2`
- `ghcr.io/drjakeberg/stream247-playout:v1.5.2`
...
6. Rehearse the target version with:
   ```bash
   ./scripts/upgrade-rehearsal.sh v1.5.2
   ```
   Before a new release tag exists, the rehearsal automatically uses the CI-published `main-<sha>` snapshot for the current commit instead of requiring `ghcr.io/...:v1.5.2` to exist already.
