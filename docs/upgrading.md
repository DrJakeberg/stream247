...
6. Update the pinned image tags in your Compose configuration, or rehearse the target version with:
   ...
   ```
   Before a new release tag exists, the rehearsal automatically uses the CI-published `main-<sha>` snapshot for the current commit instead of requiring `ghcr.io/...:v1.1.1` to exist already.
